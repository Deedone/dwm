...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > selmon->ww - TEXTW(stext) - getsystraywidth() - TEXTW(xkb_layouts[xkbGlobal.group])) {
			x =selmon->ww - TEXTW(stext) - getsystraywidth() - TEXTW(xkb_layouts[xkbGlobal.group]);
			click = ClkStatusText;
			char *text = rawstext;
			int i = -1;
			char ch;
